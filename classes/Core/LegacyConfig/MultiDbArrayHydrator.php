...
final class MultiDbArrayHydrator
{
    ...
    private static function fillEmptyValues(Config $defaultConfig, $multiDbArray)
    {
        ...
        foreach ($multiDbArray as $key => $item) {
            ...
            $dbreadonlyuser = !empty($item['dbreadonlyuser']) ? $item['dbreadonlyuser'] : $dbuser;
            $dbreadonlypass = !empty($item['dbreadonlypass']) ? $item['dbreadonlypass'] : $dbpass;
            ...
            $result[$key] = [
                ...
                'dbuser' => $dbuser,
                'dbpass' => $dbpass,
                'dbreadonlyuser' => $dbreadonlyuser,
                'dbreadonlypass' => $dbreadonlypass,
    ...
    private static function includeDefaultConfig(Config $defaultConfig, $multiDbArray)
    {
        ...
        $multiDbArray[$defaultConfigKey] = [
            ...
            'dbreadoonlyuser' => property_exists(
                $defaultConfig,
                'WFdbreadonlyuser'
            ) ? $defaultConfig->WFdbreadonlyuser : $defaultConfig->WFdbuser,
            'dbreadoonlypass' => property_exists(
                $defaultConfig,
                'WFdbreadonlypass'
            ) ? $defaultConfig->WFdbreadonlypass : $defaultConfig->WFdbpass,
            'cronjob' => true,
        ];
