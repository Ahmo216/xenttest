...
final class SubscriptionCycleArticleGateway
{
    ...
    public function findMonthlySubscriptionData(DateTimeInterface $nextFirstDay): array
    {
        $sql =
            ...
            ROUND(ROUND(
              DATEDIFF(
                CASE
                    WHEN abr.enddatum != \'0000-00-00\' AND abr.enddatum < LAST_DAY(:next_first_day)
                        THEN LAST_DAY(abr.enddatum)
              ...
              AND (
                  abr.abgerechnetbis = \'0000-00-00\'
                    OR  (abr.abgerechnetbis < :next_first_day)
                       OR (abr.abgerechnetbis < LAST_DAY(:next_first_day)
                               AND abr.preisart IN (\'monat\', \'\'))
                  )
              AND (abr.enddatum = \'0000-00-00\'  
                   OR (abr.abgerechnetbis != \'0000-00-00\' AND abr.abgerechnetbis < abr.enddatum)
                   OR (abr.abgerechnetbis = \'0000-00-00\' AND abr.startdatum < abr.enddatum)
                   OR abr.enddatum >= DATE_SUB(:next_first_day, INTERVAL 1 DAY)
                  )';
    ...
    public function findYearlySubscriptionData(DateTimeInterface $nextFirstDay): array
    {
        $sql =
            ...
            abr.id AS `subscription_article_id`, 
            IF(
              abr.abgerechnetbis IS NOT NULL AND abr.abgerechnetbis != '0000-00-00',
              DATE_ADD(abr.abgerechnetbis, INTERVAL 1 DAY),
              IF(
                abr.startdatum = LAST_DAY(abr.startdatum),
                DATE_ADD(abr.startdatum, INTERVAL 1 DAY),
            ...
            ROUND(
              DATEDIFF(
                IF(
                    ...
                    CASE 
                        WHEN abr.abgerechnetbis = LAST_DAY(abr.abgerechnetbis)
                            THEN DATE_ADD(abr.abgerechnetbis, INTERVAL IF(abr.zahlzyklus = 0, 1, abr.zahlzyklus) YEAR)
                        WHEN abr.startdatum > :next_first_day
                            THEN DATE_ADD(LAST_DAY(abr.startdatum), INTERVAL 1 DAY)
                        ELSE
                            :next_first_day
                    ...
                    CASE
                        WHEN abr.startdatum > :next_first_day
                            THEN DATE_ADD(LAST_DAY(abr.startdatum), INTERVAL 1 DAY)
                        ELSE 
                            DATE_ADD(DATE_SUB(:next_first_day, INTERVAL 1 DAY), INTERVAL IF(abr.zahlzyklus = 0, 1, abr.zahlzyklus) YEAR)
                    END
                )
                ,
                IF(
                  ...
                  CASE 
                      WHEN abr.abgerechnetbis = LAST_DAY(abr.abgerechnetbis)
                          THEN DATE_ADD(abr.abgerechnetbis, INTERVAL 1 DAY)
                      ELSE DATE_ADD(LAST_DAY(DATE_SUB(abr.abgerechnetbis,INTERVAL 1 MONTH)), INTERVAL 1 DAY)
                  END,
                  CASE 
                      WHEN abr.startdatum = LAST_DAY(abr.startdatum)
                        THEN DATE_ADD(abr.startdatum, INTERVAL 1 DAY)
            ...
            WHERE abr.preisart = 'jahr'
              AND (abr.enddatum IS NULL OR abr.enddatum = '0000-00-00' OR abr.enddatum > abr.startdatum) ";
    ...
    public function find30DaysSubscriptionData(DateTimeInterface $date): array
    {
        $sql =
            ...
            (FLOOR(    
                DATEDIFF(
                  IF(abr.enddatum != \'0000-00-00\' AND abr.enddatum < :date,
                      abr.enddatum,
                      :date
                  ),
                  IF(
