...
final class EbayListingGateway
{
    ...
    public function getArticleIdAssociatedToStagingListingBySku(string $sku, int $shopId): ?int
    {
        $values = [
            'sku'     => $sku,
            'shop_id' => $shopId,
        ...
        $sql = "SELECT e.article_id 
                FROM `ebay_staging_listing` AS `e`
                WHERE e.sku = :sku AND e.shop_id = :shop_id AND e.status = 'Aktiv'
        ...
        $sql = "SELECT v.article_id 
                FROM `ebay_staging_listing` AS `e`
                INNER JOIN `ebay_staging_listing_variant` AS `v` ON e.id = v.ebay_staging_listing_id 
                WHERE v.sku = :sku AND e.shop_id = :shop_id AND e.status = 'Aktiv'
                ORDER BY v.article_id DESC 
                LIMIT 1";
        $articleId = $this->db->fetchValue($sql, $values);
        if (!empty($articleId)) {
            return (int)$articleId;
    ...
    public function getArticleIdByLegacyForeignNumber(string $itemId, int $shopId): ?int
    {
        $sql = "SELECT a.id
                FROM `artikelnummer_fremdnummern` AS `af`
                INNER JOIN `artikel` AS `a` ON af.artikel = a.id
                WHERE LOWER(af.bezeichnung)='ebaylisting' AND af.nummer = :item_id AND af.shopid = :shop_id 
                  AND af.aktiv = 1 AND a.intern_gesperrt = 0 AND a.geloescht = 0 
                LIMIT 1";
        $values = [
            'item_id' => $itemId,
            'shop_id' => $shopId,
    ...
    public function existsReservedStockIdForItemId(string $itemId):bool{
        $sql = "SELECT e.id
                FROM `ebay_reserved_stock` AS `e`
                WHERE e.external_order_id = :item_id 
                LIMIT 1";
        $values = [
            'item_id' => $itemId,
        ];
        ...
        return !empty($articleId);
    }

    public function getArticleIdsForStockReservation(string $itemId): array{
        $sql = "SELECT e.article_id
                FROM `ebay_reserved_stock` AS `e`
                WHERE e.external_order_id = :item_id";
        $values = [
        ...
        $articleIds = $this->db->fetchAll($sql, $values);
