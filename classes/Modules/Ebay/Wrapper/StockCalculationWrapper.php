...
final class StockCalculationWrapper implements EbayStockCalculationWrapperInterface
{
    ...
    public function calculateStock($articleId, $shopId): float
    {
        ...
        $pseudoStorage = trim((string)$this->erp->GetArtikelShopEinstellung('pseudolager', $articleId, $shopId));
        ...
        if($pseudoStorage === '' && $this->erp->ModulVorhanden('pseudostorage')){
            ...
            $pseudoStorage = trim((string)$this->db->fetchValue($sql, $values));
        }
        ...
        if ($pseudoStorage !== '') {
            return $this->recalculateForMaximumStock($pseudoStorage > 0 ? floor((float)$pseudoStorage) : 0, $shopId);
        }
