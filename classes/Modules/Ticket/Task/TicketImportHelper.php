...
class TicketImportHelper
{
    ...
    public function importMessage(MailMessageInterface $message): void
    {
        ...
        $schluessel = null;
        $ticketexits = null;
        if (strpos($subject, 'Ticket #') !== false && preg_match("/Ticket #[0-9]{12}/i", $subject, $matches)) {
            $schluessel = trim(str_replace('Ticket #', '', $matches[0]));
            if (preg_match('/^[0-9]{12}$/', $schluessel)) {
                $ticketexits = $this->db->Select(
                    "SELECT `schluessel` FROM `ticket` WHERE `schluessel` LIKE '${schluessel}' AND `schluessel` != '' LIMIT 1"
                );
            }
            $schluessel = $ticketexits ?? null;
        }
        ...
        if (
            ...
            $text = str_replace(['{TICKET}', '{BETREFF}'], [$schluessel, $subject], $text);
            $betreff = str_replace(['{TICKET}', '{BETREFF}'], [$schluessel, $subject], $betreff);
