...
var TicketOverview = function ($) {
    'use strict';

    var me = {
        selector: {
            overviewTable: '#ticket_offene',
            queuePopup: '#popup-queue',
            addressPopup: '#popup-address'
        },
        storage: {
        ...
        getSelectedTickets: function () {
            me.storage.selectedTickets = [];
            $(me.selector.overviewTable).find(':checked').each(function () {
                me.storage.selectedTickets.push($(this).val());
            });
        },
        openAssingPopup: function (popuptype) {
            me.getSelectedTickets();
            if (me.storage.selectedTickets.length === 0) {
                return;
            }
            me.storage.popupOpen = '#popup-' + popuptype;
            $('#popup-' + popuptype + '-input').val('');
            $(me.storage.popupOpen).dialog({
                modal: true,
                autoOpen: true,
                minWidth: 940,
                buttons: {
                    ...
                    'ZUWEISEN': function () {
                        $.ajax({
                            url: 'index.php?module=ticket&action=offene&cmd=batch_' + popuptype,
                            type: 'POST',
                            dataType: 'text',
                            data: {
                                ticket_ids: me.storage.selectedTickets.length,
                                value: $('#popup-' + popuptype + '-input').val(),
                                type: popuptype
                            },
                            success: function () {
                                $(me.storage.popupOpen).dialog('close');
        ...
        init: function () {
            $('#button-add-queue').on('click', function () {
                me.getSelectedTickets();
                me.openAssingPopup('queue');
            });
            $('#button-add-address').on('click', function () {
                me.getSelectedTickets();
                me.openAssingPopup('address');
            });
        }
    };
