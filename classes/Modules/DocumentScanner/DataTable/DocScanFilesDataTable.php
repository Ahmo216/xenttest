...
final class DocScanFilesDataTable extends AbstractDataTableType
{
    ...
    public function configureQuery(SelectQuery $query)
    {
        $query
            ...
            ->from('datei AS d')
            ->innerJoin('datei_stichwoerter AS s', 'd.id = s.datei')
            ...
            ->joinSubSelect(
                ...
                'SELECT ds.datei, GROUP_CONCAT(ds.objekt SEPARATOR \', \') AS `objekte`
                 ...
                 INNER JOIN (SELECT `datei` FROM `docscan`) AS `doc` ON ds.datei = doc.datei
            ...
            ->joinSubSelect(
                'LEFT',
                'SELECT dvi1.datei, max(dvi1.version) AS `version` 
                FROM `datei_version` AS `dvi1`
                INNER JOIN `docscan` AS `doc` ON dvi1.datei = doc.datei
                GROUP BY dvi1.datei',
                'v2',
                'v2.datei = d.id'
            )
    ...
    public function configureFilters(FilterCollection $filters)
    {
        $closure = function (SelectQuery $query, DataTableRequest $request) {
            ...
            if ($filter['show-all'] !== 'true') {
                $query->where('zuweisungen.datei IS NULL');
            }
        };
