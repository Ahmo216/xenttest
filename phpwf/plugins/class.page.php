...
class Page 
{
  ...
  public function drawMenu($menu, $aktmodulekey, $aktmodule, $aktactionkey) {
    ...
    /**
     * If there are due payments, it shows a warning above the main page content.
     * On the first login with due payments, it shows a modal (clickByClickAssistant)
     */
    public function showDunningInformation()
    {
        $userLanguage = $this->app->User->GetParameter('wawisionuebersetzung_sprache');

        $warning = "<b>Unfortunately we have problems processing your payment.</b> Your account is still active, 
                        but we kindly ask you to get in touch with us for clarification. Please write us at 
                        <a href=\"mailto:kontakt@xentral.com\">kontakt@xentral.com</a>.";

        if ($userLanguage === "german") {
            $warning = "<b>Leider konnten wir Deine Zahlung nicht verarbeiten.</b> Dein Konto ist weiterhin aktiv, 
                            wir bitten Dich aber um Klärung. Bitte kontaktiere uns unter 
                            <a href=\"mailto:kontakt@xentral.com\">kontakt@xentral.com</a>.";
        }

        // warning above main page content
        $this->app->Tpl->Set('FIXEDNOTIFICATION', '<div class="warning warning-rounded">' . $warning . '</div>');

        // if a user has seen the modal before, don't load it again
        if ($this->app->User->GetParameter('dunningAssistantSeen') === "true") {
            return;
        }

        $this->app->ModuleScriptCache->IncludeWidgetNew('ClickByClickAssistant');

        $translationFile = './classes/Modules/Dunning/www/js/dunning-en.js';

        if ($userLanguage === "german") {
            $translationFile = './classes/Modules/Dunning/www/js/dunning-ger.js';
        }

        $this->app->ModuleScriptCache->IncludeJavascriptFiles(
            'dunningAssistant',
            [
                'body' => [$translationFile],
            ]
        );

        // modal on first page view with due payments
        $this->app->Tpl->Parse('DUNNINGASSISTANT', 'dunning_assistant.tpl');

        $this->app->User->SetParameter('dunningAssistantSeen', "true");
    }
}
