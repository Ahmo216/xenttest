...
class User 
{
  ...
  public function projectOk($projektId): ?bool
  {
    ...
    return in_array($projektId, $user['project']);
  }
  ...
  function SettingsToUserKonfiguration($settings = null)
  {
    ...
    if($settings === null) {
      $settings = (string)$this->app->DB->Select("SELECT `settings` FROM `user` WHERE `id` = {$id} LIMIT 1");
      $cacheKey = $this->getCacheKey();
  ...
  public function GetAdresse(): int
  {
    ...
    if(!empty($this->cache[$cacheKey]) && isset($this->cache[$cacheKey]['adresse'])) {
      return (int) ($this->cache[$cacheKey]['adresse'] ?? 0);
    }
    $this->loadUserRowInCacheProperty();
  ...
  function GetProjektleiter()
  {
    ...
    $result = $this->app->DB->SelectRow(
      "SELECT `parameter` 
      FROM `adresse_rolle` 
      WHERE `subjekt` = 'Projektleiter'
        AND (`bis` IS NULL OR `bis` = '0000-00-00' OR `bis` >= CURDATE()) 
  ...
  function DefaultProjekt()
  {
    ...
    if(empty($this->cache[$cacheKey]) || !isset($this->cache[$cacheKey]['projekt'])) {
      ...
      $projekt = (int)$this->cache[$cacheKey]['projekt'];
    }
    else {
      $projekt = (int)$this->cache[$cacheKey]['projekt'];
    }
    ...
    return (int)$this->app->DB->Select(
        'SELECT `standardprojekt` FROM `firma` WHERE `id` = 1 LIMIT 1'
      );
  }
  ...
  {
    $userId = $this->GetID();
    $value = $userId <= 0 ? null : $this->app->DB->Select(
      "SELECT `{$field}` FROM `user` WHERE `id` = {$userId} LIMIT 1"
    );
    switch($field) {
      case 'adresse':
        $cacheKey = $this->getCacheKey();
        $this->cache[$cacheKey][$field] = (int)$value;
        return (int)$value;
      case 'settings':
      case 'type':
      case 'username':
      case 'sprachebevorzugen':
        $cacheKey = $this->getCacheKey();
  ...
  protected function loadUserRowInCacheProperty(?int $userId = null): void
  {
    ...
    $cacheKey = $this->getCacheKey();
    $userData = $userId <= 0 ? [] : (array)$this->app->DB->SelectRow(
      "SELECT `settings`, `type`, `username`, `adresse`, `sprachebevorzugen` FROM `user` WHERE `id` = {$userId} LIMIT 1"
    );
    $userData['settings'] = $userData['settings'] ?? '';
    $userData['type'] = $userData['type'] ?? '';
    $userData['username'] = $userData['username'] ?? '';
    $userData['adresse'] = (int)($userData['adresse'] ?? 0);
    $userData['sprachebevorzugen'] = $userData['sprachebevorzugen'] ?? '';
  ...
  {
    if($addressId === null){
      $addressId = $this->GetAdresse();
    }
    $cacheKey = $this->getCacheKey();
    $addressData = $addressId <= 0 ? [] : (array)$this->app->DB->SelectRow(
      "SELECT `name`, `email`, `projekt` FROM `adresse` WHERE `id` = {$addressId} LIMIT 1"
    );
    $addressData['name'] = $addressData['name'] ?? '';
    $addressData['email'] = $addressData['email'] ?? '';
    $addressData['projekt'] = (int)($addressData['projekt'] ?? 0);
    if(!isset($this->cache[$cacheKey])) {
  ...
  protected function loadProjectsInCacheProperty(): void
  {
    ...
    foreach($projects as $projectId => $public) {
      if($public) {
        $this->cache[$cacheKey]['public_projects'][] = (int)$projectId;
      }
    }
  }
  ...
  protected function getUserProjectsByParameter($addressId, $type)
  {
    if($type==='admin' ||
      $this->app->DB->Select(
        "SELECT `id` 
        FROM `adresse_rolle` 
        WHERE (`bis` IS NULL OR `bis` = '0000-00-00' OR `bis` >= CURDATE())
          AND `adresse` = '{$addressId}' 
    ...
    return array_unique(array_map('intval', array_merge($public, $roles, $projects)));
  }
}
