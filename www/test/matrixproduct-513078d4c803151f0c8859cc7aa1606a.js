...
var MatrixproductListview = function ($) {
    ...
    var me = {
        ...
        selector: {
            ...
            popupOptionImage: '#popupoptionimage',
        ...
        editOptionImage: function(id){
            $('#optionimageid').val(id);

            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: 'index.php?module=matrixprodukt&action=artikel&cmd=listgetoptionimage',
                data: {
                    optionid: id
                },
                success: function (response) {
                    $('#optionimagetitle').val(response.data.title);
                    $('#optionimagedescription').val(response.data.description);
                    $('#optionimagefilename').html(response.data.filename);
                    $('#optionimagefile').val('');
                    $(me.selector.popupOptionImage).dialog('open');
                }
            });
        },
        ...
        optionTableAfterReload:function(){
            ...
            $(me.selector.optionTable).find('img.editoptionimage').on('click',function(){
                me.editOptionImage($(this).data('id'));
            });
        ...
        initListView: function(){
            ...
            $(me.selector.popupOptionImage).dialog(
                {
                    modal: true,
                    autoOpen: false,
                    minWidth: 940,
                    ...
                    buttons: {
                        'ABBRECHEN':function(){
                            $(me.selector.popupOptionImage).dialog('close');
                        },
                        'BILD ENTFERNEN':function(){
                            $.ajax({
                                type: 'POST',
                                dataType: 'json',
                                url: 'index.php?module=matrixprodukt&action=artikel&cmd=deleteoptionimage',
                                data: {
                                    optionid: $('#optionimageid').val()
                                }
                            });
                            $(me.selector.popupOptionImage).dialog('close');
                        },
                        'SPEICHERN': function () {
                            $(me.selector.popupOptionImage).parent().loadingOverlay('show');
                            var form_data = new FormData();
                            form_data.append('upload', $('#optionimagefile').prop('files')[0]);
                            form_data.append('optionid', $('#optionimageid').val());
                            form_data.append('title', $('#optionimagetitle').val());
                            form_data.append('description', $('#optionimagedescription').val());
                            $.ajax({
                                type: 'POST',
                                dataType: 'json',
                                ...
                                processData: false,
                                contentType: false,
                                url: 'index.php?module=matrixprodukt&action=artikel&cmd=listsaveoptionimage',
                                data: form_data,
                                success: function () {
                                    $(me.selector.optionTable).DataTable().ajax.reload();
                                    $(me.selector.articleTable).DataTable().ajax.reload();
                                    $(me.selector.popupOptionImage).parent().loadingOverlay('remove');
                                    $(me.selector.popupOptionImage).dialog('close');
                                },
                                error: function(){
                                    $(me.selector.optionTable).DataTable().ajax.reload();
