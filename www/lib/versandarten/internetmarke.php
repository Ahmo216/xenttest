...
use Xentral\Modules\DeutschePost\ProductWebServiceClient;
...
class Versandart_internetmarke extends Versanddienstleister
{
    ...
    /** @var ProductWebServiceClient */
    private $productWebServiceClient;
    ...
    function __construct($app, $id)
    {
        ...
        $this->productWebServiceClient = $app->Container->get(ProductWebServiceClient::class);
    }
    ...
    function EinstellungenStruktur()
    {
        ...
        try{
          $productOptions = [-1 => 'Im Paketmarkendialog auswählbar'] + $this->getProductList();
        }catch (ProductWebServiceException $e){
          $productOptions = [-1 => 'Fehler'];
          $this->app->Tpl->Set('MESSAGE', '<div class="error">Fehler beim aktualisieren der Produktliste: ' . $e->getMessage() . '</div>');
        }
        ...
        if ($target) {
            ...
            $product = $this->einstellungen['product'];
              if(empty($product) || $product == -1){
                try{
                  $products = $this->productWebServiceClient->getProducts();
                }catch (ProductWebServiceException $e){
                  $products = [];
                  $this->app->Tpl->Set('MESSAGE', '<div class="error">Fehler beim aktualisieren der Produktliste: ' . $e->getMessage() . '</div>');
                }
              $optionHtmls = array_map(function ($product){
                $id = $product['pplId'];
                $price = $product['price'];
                $name = $product['name'];
                return "<option value=\"{$id}\">{$name} ({$price})</option>";
              }, $products);
              $productListHtml = implode('', $optionHtmls);
              $productListHtml = "<tr><td>Produkt: </td><td><select name=\"product\">{$productListHtml}</select></td></tr>";
            }else{
              $productListHtml = '<tr><td><select name="product" style="visibility: hidden"><option value="' . $product . '">Product</option></select></td></tr>';
            }

            $this->app->Tpl->Set('PRODUCT_LIST', $productListHtml);
  ...
  <soapenv:Body>
    ...
    {
        $price = $this->productWebServiceClient->getProductPrice($id);
