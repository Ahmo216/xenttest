...
class Amainvoice
{
  ...
  {
    return ['ftp', 'createorder', 'turnoverthresholdactive', ];
  }
  ...
  public function AmainvoiceList()
  {
    ...
    if($this->app->Secure->GetPOST('save')){
      ...
      try{
        $this->service->syncBasicTaxDataForCurrentDateIfNotExists();
      }
      catch (Exception $e) {
        return RedirectResponse::createFromUrl('index.php?module=amainvoice&action=list');
      }
      return RedirectResponse::createFromUrl('index.php?module=amainvoice&action=list');
    }
    ...
    $this->app->Tpl->Set('BEFORETHRESHOLD', '<!--');
    $this->app->Tpl->Set('AFTERTHRESHOLD', '-->');
    if($this->service->hasTurnoverThresholdModule()) {
      $missingCountries = $this->service->getMissingThresholdCountries();
      if(!empty($missingCountries)) {
        $this->app->Tpl->Add(
          'MESSAGE',
          '<div class="info">
          {|AmaInvoice meldet Länder die nicht in der Lieferschwellen-App gepflegt sind: |}'
          .implode(', ', $missingCountries)
          .' <a target="_blank" href="index.php?module=lieferschwelle&action=list">{|zur App|}</a>
          </div>'
        );
      }
    }
  ...
  public function Install()
  {
    ...
    $this->app->erp->CheckTable('amainvoice_turnoverthreshold');
    $this->app->erp->CheckColumn('country_code', 'varchar(3)', 'amainvoice_turnoverthreshold', 'NULL DEFAULT NULL');
    $this->app->erp->CheckColumn('vat_number', 'varchar(32)', 'amainvoice_turnoverthreshold', 'NULL DEFAULT NULL');
    $this->app->erp->CheckColumn('is_active', 'tinyint(1)', 'amainvoice_turnoverthreshold', 'NULL DEFAULT NULL');
    $this->app->erp->CheckColumn('active_since', 'date', 'amainvoice_turnoverthreshold', 'NULL DEFAULT NULL');
    $this->app->erp->CheckColumn('turnoverthreshold_id', 'int(10)', 'amainvoice_turnoverthreshold', 'NULL DEFAULT NULL');

    $this->app->erp->RegisterHook('turnoverthreshold_list', 'amainvoice' ,'TurnoverthresholdList');
  }

  public function TurnoverthresholdList(): void
  {
    $threshHolds = $this->app->DB->SelectArr(
      "SELECT `id`, `country_code`, `vat_number`, `is_active`, `turnoverthreshold_id`, 
       IF(`active_since` IS NULL, NULL, DATE_FORMAT(`active_since`, '%d.%m.%Y')) AS `active_since`
      FROM `amainvoice_turnoverthreshold`"
    );
    if(empty($threshHolds)){
      return;
    }
    $countryCodes = [];
    foreach ($threshHolds as $threshHold) {
      if(empty($threshHold['is_active'])) {
        continue;
      }
      $countryCodes[] = $threshHold['country_code'];
    }
    $this->app->Tpl->Add(
      'TAB1',
      '<script id="amainvoice-turnoverthreshold-data" type="application/json">'
      . json_encode($threshHolds)
      . '</script>'
    );
    $this->app->ModuleScriptCache->IncludeJavascriptFiles(
      'TurnoverThreshold',
      [
        './classes/Modules/AmaInvoice/www/js/amainvoice_turnoverthreshold.js',
      ]
    );
    ...
    $this->app->Tpl->Add(
      'MESSAGE',
      '<div class="info">{|Das Modul AmaInvoice verwendet Lieferschwellen in |}'
