...
class Shopimporter_Woocommerce extends ShopimporterBase
{
  ...
  private $protokoll;
  protected $useCouponLineForDiscountName;
  ...
  public function parseOrder($wcOrder) {
    ...
    if(!empty((string)$wcOrder->currency)){
      $order['waehrung'] = (string)$wcOrder->currency;
    }
    //
    ...
    if($discount_total != 0) { // Discount was applied to this order
      ...
      if($discount_tax == 0) {
        ...
        $order['rabattsteuer'] = round($discount_tax / ($discount_total / 100));
      }
      ...
      if($this->useCouponLineForDiscountName && is_array($couponLine) && isset($couponLine[0]) && (String)$couponLine[0]->code !== '') {
    ...
    if(!empty($wcOrder->shipping_lines[0]->total)){
        $shippingTaxRate = round($wcOrder->shipping_lines[0]->total_tax / ($wcOrder->shipping_lines[0]->total / 100));
        $order['portosteuersatz'] = 'normal';
        if($shippingTaxRate <= 10){
            $order['portosteuersatz'] = 'ermaessigt';
        }
        if($wcOrder->shipping_lines[0]->total_tax <= 0){
            $order['portosteuersatz'] = 'befreit';
        }
    }
  ...
  function parseItem($wcOrderItem){
    ...
    $subTotal = (float)$wcOrderItem->subtotal;
    $quantity = (float)$wcOrderItem->quantity;
    $taxValue = (float)$wcOrderItem->subtotal_tax;
    $priceType = 'price_netto';
    $priceValue = $subTotal / $quantity;

    if($this->priceType === 'grosscalculated'){
      $priceType = 'price';
      $priceValue = ($subTotal + $taxValue) / $quantity;
    }
    ...
    if(!empty($wcOrderItem->subtotal_tax)){
        $orderItem['steuersatz'] = round($taxValue / ($subTotal/100),1);
    }
  ...
  public function ImportSendList() {
    ...
    for($i=0;$i<count($tmp);$i++){
      ...
      $chosenCats = array();
      if($this->enableCategoryExport && (isset($tmp[$i]['kategorien']) || isset($tmp[$i]['kategoriename']))){
  ...
  public function getKonfig($shopid, $data)
  {
    ...
    $this->useCouponLineForDiscountName = !(bool)$preferences['felder']['useDefaultDiscountName'];
    ...
    return
      array(
        ...
        'felder'=>array(
          ...
          'useDefaultDiscountName'=>array('typ'=>'checkbox','bezeichnung'=>'Rabattbezeichnung immer aus Artikel beziehen:'),
