...
?>
<?php
use Xentral\Components\Database\ReadOnlyDataBaseCheckService;
...
class Systemhealth {
  ...
  public function SystemHealthIconBoxedStart(&$boxCount)
  {
    ...
    $this->app->Tpl->Add(
      'ICONBOXESHOOK1',
      '<li>
        <a title="' . __('System') . '" href="index.php?module=systemhealth&action=list">					
          <span class="icon-box">
  ...
  public function fillEntries()
  {
    ...
    try{
      ...
      /** @var ReadOnlyDataBaseCheckService $readOnlyDataBaseCheckService */
      $readOnlyDataBaseCheckService = $this->app->Container->get('ReadOnlyDataBaseCheckService');
      if(!$readOnlyDataBaseCheckService->isReadonlyDbAvailable()) {
        $this->changeStatus('database', 'read_only_database', 'warning', 'Readonly User ist nicht aktiviert');
      }
      elseif(!$readOnlyDataBaseCheckService->candDbRead()) {
        $this->changeStatus('database', 'read_only_database', 'warning', 'Readonly User hat keine Leserechte');
      }
      elseif($readOnlyDataBaseCheckService->canDbInsert()) {
        $this->changeStatus('database', 'read_only_database', 'warning', 'Readonly User hat Schreibrechte');
      }
      elseif(!$isReadonlyActive) {
        $this->changeStatus('database', 'read_only_database', 'warning', 'Readonly Verfügbar aber inaktiv');
      }
      else {
        $this->changeStatus('database', 'read_only_database', 'ok', 'Readonly User aktiviert');
      }
    }
    catch (Exception $e) {
  ...
  public function getValues()
  {
    return [
      ...
      'database' => [
        ...
        'read_only_database' => 'Read Only DB',
