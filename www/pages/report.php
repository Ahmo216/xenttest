...
use Xentral\Components\Database\ReadOnlyDbQueryKiller;
...
class Report
{
    ...
    /** @var ReadOnlyDbQueryKiller */
    protected $readOnlyDbQueryKiller;

    /** @var TemplateParser $template */
    protected $template; //legacy template

    /** @var Request $request */
    protected $request;

    /** @var array $parameterNameBlacklist */
    protected $parameterNameBlacklist = [
    ...
    {
        $this->app = $app;
        $this->readOnlyDbQueryKiller = $this->app->Container->get('ReadOnlyDbQueryKiller');
        if ($intern) {
            return;
        }
    ...
    public function TableSearch($app, $name, $erlaubtevars)
    {
        ...
        switch ($name) {
            ...
            case 'report_table':
                ...
                if ($this->readOnlyDbQueryKiller === null) {
                  $this->readOnlyDbQueryKiller = $this->app->Container->get('ReadOnlyDbQueryKiller');
                }
                $this->killReadOnlyQueries();
                /** @var Database $db */
                $db = $this->app->Container->get('ReadOnlyDatabase');
    ...
    public function beforeJsCssHook()
    {
      ...
      if(!$this->app->erp->RechteVorhanden('report', 'view')) {
        return;
      }
   ...
   */
    ...
    protected function killReadOnlyQueries(): void
    {
        $killedQueries = $this->readOnlyDbQueryKiller->tryKillLongRunningQueries();
        if(count($killedQueries) === 0) {
            return;
        }
        /** @var Logger $logger */
        $logger = $this->app->Container->get('Logger');
        $logger->error('Read Only Queries aborted', $killedQueries->getProcessListQueries());
    }
}
