...
// $palette is an iterator on colors sorted by pixel count
      ...
      if($error=='')
      {
        ...
        for($i = 1; $i <= 5; $i++) {
          ...
          if(isset($firmendaten_werte_spalten)) {
            ...
            if(isset($firmendaten_werte_spalten[$n8]) && $firmendaten_werte_spalten[$n8]['wert'] != $v8)
            {
              $this->app->DB->Update("UPDATE firmendaten_werte SET wert = '$v8' WHERE id = '".$firmendaten_werte_spalten[$n8]['id']."' LIMIT 1");
              unset($firmendaten_werte_spalten[$n8]);
              $checkDoublettes[] = $n8;
            }
          }
        }
  ...
  function fillFormFromDB($id)
  {
    ...
    }else
    {
      ...
      /** @var Verbindlichkeit $liabilityObject */
      $liabilityObject = $this->app->loadModule('verbindlichkeit');
      $liabilityColumns = [];
      if($liabilityObject !== null && method_exists($liabilityObject, 'getLiabilityColumns')) {
        $liabilityColumns = $liabilityObject->getLiabilityColumns();
      }
      for($i = 1; $i <= 5; $i++)
      {
        ...
        foreach($liabilityColumns as $k => $v)
        {
          $this->app->Tpl->Add('LIABILITYTABLECOL'.$i,'<option value="'.$k.'"'.($k == $data[0]['liabilitytablecol'.$i]?' selected="selected" ':'').'>'.$v.'</option>');
        }
  ...
  public function checkPaymentModule($payment, $onlyNew = false) {
    ...
    elseif($payment === 'rechnung' || $this->app->erp->ModulVorhanden("zahlungsweise_{$payment}")) {
      /** @var Zahlungsweisenmodul $paymentMethodInvoiceModule */
      $paymentModule = preg_replace('/[^a-zA-Z0-9\_]/', '', $payment);
      $class = "Zahlungsweise_{$paymentModule}";
      $path = dirname(__DIR__)."/lib/zahlungsweisen/{$paymentModule}.php";
      if(!class_exists($class) && is_file($path)) {
        include $path;
      }
      if(class_exists($class)) {
        $paymentMethodInvoiceModule = new $class($this->app, $id);
        if(method_exists($paymentMethodInvoiceModule, 'updateFromCompanySettings')) {
          $paymentMethodInvoiceModule->updateFromCompanySettings($id);
        }
      }
    }
