...
class Shopimporter_Shopware extends ShopimporterBase
{
    ...
    public $kategorien;
    public $groups;
    public $ignoreartikelbeschreibung;
    public $useorderid;
    protected $protokoll;
    /**
     * @var Application
        ...
        if (count($tmp) > 0) {
            foreach ($tmp as $key => $value) {
                ...
                foreach ($value as $column => $cvalue) {
                    switch ($column) {
                        case "name":
                            $this->app->DB->Update("UPDATE s_emarketing_partner SET company='$cvalue' WHERE id='$checkid' LIMIT 1");
                            break;
                        case "netto":
                            $this->app->DB->Update("UPDATE s_emarketing_partner SET percent='$cvalue' WHERE id='$checkid' LIMIT 1");
                            break;
                        case "strasse":
                            $this->app->DB->Update("UPDATE s_emarketing_partner SET street='$cvalue' WHERE id='$checkid' LIMIT 1");
                            break;
                        case "email":
                            $this->app->DB->Update("UPDATE s_emarketing_partner SET email='$cvalue' WHERE id='$checkid' LIMIT 1");
                            break;
                        case "telefax":
                            $this->app->DB->Update("UPDATE s_emarketing_partner SET fax='$cvalue' WHERE id='$checkid' LIMIT 1");
                            break;
                        case "telefon":
                            $this->app->DB->Update("UPDATE s_emarketing_partner SET phone='$cvalue' WHERE id='$checkid' LIMIT 1");
                            break;
                        case "ort":
                            $this->app->DB->Update("UPDATE s_emarketing_partner SET city='$cvalue' WHERE id='$checkid' LIMIT 1");
                            break;
                        case "plz":
                            $this->app->DB->Update("UPDATE s_emarketing_partner SET zipcode='$cvalue' WHERE id='$checkid' LIMIT 1");
                            break;
                        case "land":
                            $this->app->DB->Update("UPDATE s_emarketing_partner SET country='$cvalue' WHERE id='$checkid' LIMIT 1");
                            break;
                    }
                }
            }
    ...
    public function ImportGetArticle()
    {
        ...
        if (!isset($result['data']['name'])) {
            ...
            if (isset($tmp['nummerintern']) && !empty($result['data']['details']) && is_array($result['data']['details']) && count($result['data']['details']) > 0) {
                ...
                if (isset($data['herstellernummer'])) {
                    unset($data['herstellernummer']);
                }
                foreach ($data as $k => $v) {
                    if (strpos($k, 'freifeld_') === 0) {
                        unset($data[$k]);
                    }
                }
                foreach ($result['data']['details'] as $vdetail) {
                    if ($vdetail['number'] == $result['data']['mainDetail']['number']) {
                        ...
                        if (isset($vdetail['shippingTime']) && $vdetail['shippingTime'] != '') {
                            $arrayOfArticles[count($arrayOfArticles) - 1]['lieferzeitmanuell'] = $vdetail['shippingTime'];
                        }
                        if (isset($vdetail['ean']) && $vdetail['ean'] != '') {
                            $arrayOfArticles[count($arrayOfArticles) - 1]['ean'] = $vdetail['ean'];
                        }
                        if (isset($vdetail['supplierNumber']) && $vdetail['supplierNumber'] != '') {
                            $arrayOfArticles[count($arrayOfArticles) - 1]['herstellernummer'] = $vdetail['supplierNumber'];
                        }
                        if (isset($vdetail['weight']) && $vdetail['shippingTime'] > 0) {
                            $arrayOfArticles[count($arrayOfArticles) - 1]['gewicht'] = $vdetail['weight'];
                        }
                        if (isset($vdetail['active'])) {
                            $arrayOfArticles[count($arrayOfArticles) - 1]['aktiv'] = $vdetail['active'];
                        }
    ...
    {
        $params = array('useNumberAsId' => true);
        $result = $this->adapter->get('variants/' . $sku, $params);
    ...
    protected function getProductIdBySku(string $sku): ?string
    {
        $params = array('useNumberAsId' => true);
        $result = $this->adapter->get('articles/' . $sku, $params);
        if (empty($result['data']['id'])) {
            return null;
        }
    ...
    protected function exportStockForVariant(string $sku, int $stock,int $active): bool
    {
        ...
        $updateVariantInStock = array(
            'inStock' => $stock,
            'attributeVisionPhysicalStockForSale' => $stock,
            'active' => $active,
        );
        $result = $this->adapter->put('variants/' . $variantId, $updateVariantInStock);
        if (!is_array($result)) {
            $this->ShopwareLog("Shopware Lagerzahlenübertragung für Artikel: $sku fehlgeschlagen", print_r($result, true));
            return false;
        }
    ...
    public function ImportSendListLager() :int
    {
        ...
        $ctmp = count($tmp);
        for ($i = 0; $i < $ctmp; $i++) {
            $sku = $tmp[$i]['fremdnummer'] ?: $tmp[$i]['nummer'];
            $lastStock = (int)$tmp[$i]['restmenge'];
            $active = (int)empty($tmp[$i]['inaktiv']);
            $shippingTime = $tmp[$i]['lieferzeitmanuell'];
            $stock = $tmp[$i]['anzahl_lager'];
            $pseudoStock = trim($tmp[$i]['pseudolager']);
    ...
    public function getCategoryParentId($categoryData, &$kategoriebaumid)
    {
    ...
    public function ImportSendList()
    {
        ...
        for ($i = 0; $i < $ctmp; $i++) {
            ...
            if ($tmp[$i]['ausverkauft']) {
                $lageranzahl = 0;
                $tmp[$i]['anzahl_lager'] = 0.0;
                $tmp[$i]['pseudolager'] = 0.0;
            }
    ...
    public function GetImagesIdsbyNummer($nummer)
    {
        $params = array(
            'useNumberAsId' => true
        );
    ...
    public function ImportGetAuftrag()
    {
        ...
        if (!$warenkorb['subshop']) {
            $warenkorb['subshop'] = $result['data']['customer']['shopId'];
        }
        ...
        if (isset($result['data']['billing']['fax'])) {
