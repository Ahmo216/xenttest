...
use Xentral\Modules\SepaHbciFints\SepaXmlCreatorFactory;
...
class sepa extends LiveimportBase
{
  ...
  public function afterSavePaymentTransaction($paymentTransactionId)
  {
    ...
    if(!empty($json['iban']) || !empty($json['bic'])) {
      $paymentInfo = '';
      if(!empty($json['iban']) && !empty($json['bic'])) {
        $paymentInfo = "BIC: {$json['bic']}<br />IBAN: {$json['iban']}";
      }
      elseif(!empty($json['iban'])) {
        $paymentInfo = "IBAN: {$json['iban']}";
      }
      elseif(!empty($json['bic'])) {
        $paymentInfo = "BIC: {$json['bic']}";
      }
      $paymentInfo = $this->app->DB->real_escape_string($paymentInfo);
      $this->app->DB->Update(
        "UPDATE `payment_transaction` SET `payment_info` = '{$paymentInfo}' WHERE `id` = {$paymentTransactionId}"
      );
    }
    if(empty($json['betrag'])) {
      return;
  ...
  public function createXmlByJson($paymentTransactions, $cmd, $inhaber, $iban, $bic, $glaeubiger)
  {
    ...
    $executionDate = $this->getExecutionDate($cmd);
    $paymentInfo = $this->getPaymentInfo($cmd);

    /** @var SepaXmlCreatorFactory $creatorFactory */
    $creatorFactory = $this->app->Container->get(SepaXmlCreatorFactory::class);
    $directDebit = $creatorFactory->createDirectDebitXmlCreator(
      $executionDate,
      $inhaber,
      $iban,
      $bic,
    ...
    foreach($paymentTransactions as $paymentTransaction) {
      ...
      if($waehrung !== 'EUR'){
        $this->app->Tpl->Set(
          'MESSAGE',
          "<div class=\"error\">{|Für SEPA darf nur Euro als Währung verwendet werden.|}</div>");
        return;
      }
      ...
      $directDebit->addTransfer(
        ...
        $adr_inhaber,
        $mandatsreferenz,
        new DateTime($mandatsreferenzdatum),
        $verwendungszweck
      );
    }
   ...
   * @return DateTime
   */
  protected function getExecutionDate(string $marker)
  {
    $executionDate = new DateTime();

    if($marker === 'erste'){
      $executionDate->add(new DateInterval('P10D'));
    }else if($marker === 'folge'){
      $executionDate->add(new DateInterval('P6D'));
    }else{
      $executionDate->add(new DateInterval('P10D'));
    }
   ...
   * @return string
   */
  protected function getPaymentInfo(string $marker)
  {
    $paymentInfo = PaymentInformation::S_ONEOFF;
    if($marker === 'erste'){
      $paymentInfo = PaymentInformation::S_FIRST;
    }else if($marker === 'folge'){
      $paymentInfo = PaymentInformation::S_RECURRING;
    }else if($marker === 'letzte'){
      $paymentInfo = PaymentInformation::S_FINAL;
    }
    return $paymentInfo;
  }
