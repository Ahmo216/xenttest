  ...
  function ExtendsForm()
  {
    ...
    if($this->app->Secure->GetPOST('speichern')!='') {
      ...
      if($this->form->CallbackAndMandatorycheck(true))
      {
        ...
        {
          $supplierNumber = explode(' ', $this->app->Secure->GetPOST('lieferant'));
          $this->app->DB->Update("UPDATE `auftrag` SET `lieferantkdrnummer` = '".reset($supplierNumber)."' WHERE `id` = '{$id}' LIMIT 1");
        }else{
          $customerNumber = explode(' ', $this->app->Secure->GetPOST('adresse'));
          $this->app->DB->Update("UPDATE `auftrag` SET `lieferantkdrnummer` = '".reset($customerNumber)."' WHERE `id` = '{$id}' LIMIT 1");
        }
        $modus = 'speichern';
