...
class WidgetAngebot extends WidgetGenAngebot 
{
  ...
  function ExtendsForm()
  {
    ...
    if($this->app->Secure->GetPOST('speichern')!='') {
      ...
      if(empty($before['schreibschutz']) && $projektdanach != $projektabkuerzung){
        ...
        if(!empty($projektdanach)){
          $projectDefaultData = $this->app->DB->SelectRow("SELECT `zahlungsweise`, `versandart` FROM `projekt` WHERE `id`= '{$projektdanach}' LIMIT 1");
          ...
          if(!empty($projectDefaultData['versandart']) && $this->form->CallbackAndMandatorycheck(true)){
            $overwriteShippingMethod = $projectDefaultData['versandart'];
          }
    ...
    $projekt = $projectRow['projekt'];
    if($projekt) {
      if($this->app->DB->Select("SELECT count(id) FROM lager") > 1
        || $projectRow['standardlager'] > 0) {
        $kommissionierverfahren = $projectRow['kommissionierverfahren'];
    ...
    $this->form->NewField($field);
    if(!empty($overwriteShippingMethod)){
      $this->form->HTMLList['versandart']->htmlvalue = $overwriteShippingMethod;
      $this->form->HTMLList['versandart']->dbvalue = $overwriteShippingMethod;
    }
