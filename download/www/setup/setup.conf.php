<?php
  ...
  $setup[2]['fields']['WFdbreadonlyuser'] = 'Readonly-Benutzername';
  $setup[2]['fields']['WFdbreadonlypass']['text'] = "Readonly-Passwort";
  $setup[2]['fields']['WFdbreadonlypass']['type'] = "password";
  ...
  function pruefe($step)
  {
    if($step == 1)
    {
      ...
      if(!hasExectuableExtension('zip')) {
        $tmp['status'] = 'warning';
        $tmp['text'] = 'zip Executable nicht installiert';
        $ret[] = $tmp;
      }
      else {
        $tmp['status'] = 'ok';
        $tmp['text'] = 'zip Executable verf&uuml;gbar';
        $ret[] = $tmp;
      }
