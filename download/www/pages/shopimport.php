...
class Shopimport 
{
  ...
  public function setShopImportedOrderTrash($shopImportedOrderId) {
    $statement = sprintf(
      'SELECT `shopid`, `bestellnummer`, `extid` FROM `shopimport_auftraege` WHERE `id` = %d LIMIT 1',
      $shopImportedOrderId
    );
    $orderToTrashInformation = $this->app->DB->SelectArr($statement);
    $orderToTrashInformation = reset($orderToTrashInformation);

    $this->app->erp->RunHook('ShopimportTrashOrder', 3, $orderToTrashInformation['shopid'], $orderToTrashInformation['extid'], $orderToTrashInformation['bestellnummer']);
