...
class Shopimporter_Shopware6 extends ShopimporterBase
{
    ...
    public function shopwareRequest($method, $endpoint, $data = '', $headerInformation = [], $apiVersion = 'v2')
    {
        ...
        if (!empty($data)) {
            ...
            $payloadLength = strlen($jsonData);
            $headerInformation[] = 'Content-Length: '.$payloadLength;
        }
    ...
    protected function shopwareToken()
    {
        ...
        if (!empty($result['token'])) {
            $isTokenExpired = $this->isTokenExpired($this->tokenExpiresAfter);
            ...
            if (!empty($result['refresh_token'])) {
                $refreshedResult = $this->generateTokenByRefreshToken($result['refresh_token']);
                if ($refreshedResult['success'] === true) {
                    $result['token'] = $refreshedResult['token'];
                    return $result;
                }
            }
            return $result;
     ...
     */
    private function setTokens(array $curlResponse) : void
    {
        $this->accessToken = $curlResponse['access_token'];
        $this->refreshToken = $curlResponse['refresh_token'];
        $this->setTokenExpiresAfter($curlResponse['expires_in']);
    }
     ...
     * @return array
     */
    protected function generateTokenByRefreshToken(string $refreshToken) : array
    {
        $result = [];
        $result['success'] = false;
     ...
     * @return $this
     */
    private function setTokenExpiresAfter(int $expiresInSeconds): self
    {
        $expiresAfter = date("Y-m-d H:i:s", (time() + $expiresInSeconds));
        $this->tokenExpiresAfter = $expiresAfter;

        return $this;
    }

    /**
     * @param string $tokenExpiresAfter
     * @return bool
     */
    private function isTokenExpired(string $tokenExpiresAfter): bool
    {
        $now = time();
        $expireTime = strtotime($tokenExpiresAfter);
    ...
    public function getKonfig($shopid, $data)
    {
        ...
        $this->enableCategoryExport = !empty($importerSettings['kategorienuebertragen']);
        $this->enableDescriptionExport = empty($importerSettings['nurpreise']);
        ...
        $this->UserName = $settings['felder']['shopwareUserName'];
        $this->Password = $settings['felder']['shopwarePassword'];
        ...
        $this->defaultRuleName = $settings['felder']['shopwareDefaultRuleName'];
        $this->statesToFetch = $settings['felder']['statesToFetch'];
        $this->deliveryStatesToFetch = $settings['felder']['deliveryStatesToFetch'];
        $this->transactionStatesToFetch = $settings['felder']['transactionStatesToFetch'];
        $this->salesChannelToFetch = $settings['felder']['salesChannelToFetch'];
        $this->orderSearchLimit = $settings['felder']['orderSearchLimit'];
        $this->freeFieldOption = $settings['felder']['shopwareFreeFieldOption'];
        $this->propertyOption = $settings['felder']['shopwarePropertyOption'];
        $this->shopwareDefaultSalesChannel = $settings['felder']['shopwareDefaultSalesChannel'];
        $this->shopwareMediaFolder = $settings['felder']['shopwareMediaFolder'];
        ...
        $this->useApiVersionGreater640 = $settings['felder']['shopwareApiVersionGreater640'] === '1';

        $this->client = $this->app->Container->get('Shopware6Client');
        $this->client->useApiVersionGreater640($this->useApiVersionGreater640);
            ...
            [
                ...
                'felder' => [
                    ...
                    'shopwareApiVersionGreater640' => [
                        'typ' => 'checkbox',
                        'bezeichnung' => '{|Shop Version >= 6.4.0|}:',
                    ],
                    'shopwareUserName' => [
                        'typ' => 'text',
    ...
    public function ImportSendList()
    {
        ...
        foreach ($articleList as $article) {
            ...
            $bulkPrices = $this->getPricesFromArray($article['staffelpreise_standard'] ?? []) +
                $this->getPricesFromArray($article['staffelpreise_gruppen'] ?? []);

            if (!empty($bulkPrices)) {
                $this->deleteOldBulkPrices($articleIdShopware);
            }
            if (!empty($bulkPrices)) {
                $this->exportBulkPrices($articleIdShopware, $bulkPrices);
            }
    ...
    protected function exportBulkPrices(string $productId, array $bulkPrices): void
    {
        foreach ($bulkPrices as $priceRuleName => $pricesForSpecificQuantities) {
            foreach ($pricesForSpecificQuantities as $startingQuantity => $prices) {
                $this->saveBulkPricesToShop($productId, $priceRuleName, $startingQuantity, $prices);
            }
        }
    }
    ...
    protected function exportTranslationsForArticle(array $article, string $articleIdShopware): void
    {
        ...
        if ($this->enableDescriptionExport) {
            ...
            $preparedTranslations['GB']['description'] = $this->prepareDescription($article['uebersicht_en']);
        }
    ...
    protected function mediaToExport($internalArticleData, $articleIdShopware)
    {
        ...
        foreach ($internalArticleData['Dateien'] as $internalFile) {
            ...
            $url = $this->ShopUrl . ($this->useApiVersionGreater640 ? '' : 'v2/').'_action/media/' . $mediaId . '/upload?extension=' . $extension . '&fileName=' . $filename;
            $ch = curl_init();
            $setHeaders = [
    ...
    protected function propertiesToExport($internalArticle, $articleIdShopware): array
    {
        ...
        foreach ($propertiesToAdd as $propertyDefaultName => $countryIsoToPropertyTranslation) {
            ...
            $hasPropertyGroupId = $this->checkPropertyGroupId($propertyDefaultName);
            if (!$hasPropertyGroupId) {
                continue;
            }

            $propertyGroupId = $this->getPropertyGroupByDefaultName($propertyDefaultName);
        ...
        if (!empty($articleIdShopware)) {
            ...
            foreach ($existingProperties['data'] as $existingProperty) {
                if (!in_array($existingProperty['id'], $propertiesToExport, false)) {
    ...
    {
        return $this->propertyGroups[$propertyDefaultName];
    }
    ...
    {
        return strpos($name, 'customField_') === 0 ||
    ...
    {
        return strpos($name, 'property_') === 0 ||
            stripos($name, 'shopware6_') !== false;
    }
        ...
        foreach ($internalSpecialFields as $fieldName => $fieldValue) {
            switch (strtolower($fieldName)) {
                ...
                case 'shopware6_pack_unit_plural':
                    ...
                    break;
                case 'shopware6_min_purchase':
                    $systemFields['minPurchase'] = (int)$fieldValue;
                    break;
                case 'shopware6_purchase_steps':
                    $systemFields['purchaseSteps'] = (int)$fieldValue;
                    break;
                case 'shopware6_max_purchase':
                    $systemFields['maxPurchase'] = (int)$fieldValue;
                    break;
            }
        }
    ...
    protected function exportVariants($article, $articleIdShopware, $currencyId): bool
    {
        ...
        foreach ($article['matrix_varianten']['gruppen'] as $propertyGroupName => $internalPropertyGroupValues) {
            ...
            foreach ($internalPropertyGroupValues as $internalPropertyGroupValue => $valueNotNeeded) {
                $optionId = $this->getPropertyOptionId($propertyGroupId, $internalPropertyGroupValue);
                if ($optionId === null) {
                    $optionId = $this->createPropertyOption($propertyGroupId, $internalPropertyGroupValue);
                }
                $internalGroupPropertiesToShopwareId[$propertyGroupName][$internalPropertyGroupValue] = $optionId;
            }
        ...
        foreach ($article['artikel_varianten'] as $variant) {
            ...
            $bulkPrices = $this->getPricesFromArray($variant['staffelpreise_standard'] ?? []) +
                $this->getPricesFromArray($variant['staffelpreise_gruppen'] ?? []);
    ...
    protected function sendInvoce($extOrderId)
    {
        ...
        if (!empty($invoiceData['belegnr'])) {
            ...
            $accessToken = $this->shopwareToken();
            $url = $this->ShopUrl . ($this->useApiVersionGreater640 ? '' : 'v2/').'_action/document/' . $documentId . '/upload?_response=true&extension=pdf&fileName=' . $documentNumber;
