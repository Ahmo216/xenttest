...
<input type="button" value="{|Neue Zeit buchen|}" style="width:100%" class="btnGreenNew" onclick="OpenMode(DateFormat.format.date(jQuery.now(), 'yyyy-MM-dd HH:mm:ss'),DateFormat.format.date(jQuery.now(), 'yyyy-MM-dd HH:mm:ss'))"><br>
[FORMULARANSICHT]
</fieldset>
  ...
  loading: function(isLoading, view) {
    ...
    if(isLoading && myView!=null) {
      ...
      if(myViewDate!=null){
        ...
        var day = DateFormat.format.date(mydate, 'd');
...
var OpenMode = function(start,end) {
    ...
    if(start==end)
    {
      ...
      $('#editZeiterfassung').find('#bisZeit').val(DateFormat.format.date(end, "HH:mm"));
    }

    $('#editZeiterfassung').find('#eventid').val('');
    $('#editZeiterfassung').find('#datum').val(DateFormat.format.date(start, "dd.MM.yyyy"));
    $('#editZeiterfassung').find('#vonZeit').val(DateFormat.format.date(start, "HH:mm"));
