...
function generate(type, text) {
    if (type === 'chatbox') {
        var $topmenu = $('ul#topmenu'),
            anzchat = $topmenu.find('.chatbox').first().text();

        if (anzchat != text) {
            $topmenu.find('.chatbox').first().text(text);
            $topmenu.find('.chatbox').first().toggleClass('nachrichtenboxzahl_red', true);
            ...
            $topmenu.find('.chatbox').first().toggleClass('nachrichtenboxzahl', false);
        }
    }
}
...
var table_filter = {
    init: function () {
        ...
        $tableFilter.find('input[type="text"]').css({
            width: 200
        });

        $tableFilter.find('select').css({
            padding: '5px',
            width: 200,
            'border-radius': 0
        });
        ...
        $tableFilter.find('.smallInput').css({
            width: 125
        });
...
var App = {
    loading: {
        open: function (callback) {
            var $loaderApp = $('.loader_app');

            if ($loaderApp.length === 0) {
                App.loading.create();
            }
...
$(document).ready(function () {
    ...
    // Wir befinden uns nicht in der Profisuche > Nichts tun
    if ($customTaxRateSwitch.length === 0) {
        return;
    }
...
function DialogZwischenlager(value) {
    ...
    if (parseFloat(menge.replace(',', '.')) > 0) {
        window.location.href = 'index.php?module=lager&action=bucheneinlagern&cmd=zwischenlager&id=' + value + '&menge=' + menge;
    } else return false;
}
...
function DokumentAbschicken(modul, id, action) {
    ...
    $('<iframe id="externalSite" class="externalSite" src="index.php?module=' + modul + '&action=abschicken&id=' + id + '" width="1000"/>').dialog({
        title: 'Abschicken',
        autoOpen: true,
        width: 1100,
        height: 800,
...
$(document).ready(function () {
    ...
    if ($clock && $clock.clock !== undefined) {
        $clock.clock({
