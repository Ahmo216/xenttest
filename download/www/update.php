...
require_once ROOT_DIR . 'xentral_autoloader.php';
const LARAVEL_BOOSTRAP_CACHE_DIR = ROOT_DIR . 'bootstrap/cache/';
...
class erpAPI_Update
{
    ...
    public function calledOnceAfterLogin()
    {
    }

    public function Firmendaten($value)
    {
        $result = $this->app->DB->Select("SELECT `wert` FROM `firmendaten_werte` WHERE `name` = '{$value}' LIMIT 1");
    ...
    public function ClearDataBeforeOutput($text)
    {
...
class Md5Dateien
{
    ...
    public function __construct($quellverzeichnis)
    {
        $this->getVerzeichnis($quellverzeichnis, '', 0, '');
    }

    public function getVerzeichnis($quellverzeichnis, $zielverzeichnis, $lvl, $relativ)
    {
...
class UpdateDB
{
    ...
    public function __construct($dbhost, $dbname, $dbuser, $dbpass, &$app = '', $dbport = 3306)
    {
        $this->app = &$app;
        $this->dbname = $dbname;
    ...
    {
        return mysqli_result(mysqli_query($this->connection, $sql), 0);
    }
    ...
    public function InsertArr($tablename, $pkname, $ArrCols)
    {
...
class UpgradeClient
{
    ...
    public function TestModul($modul)
    {
    ...
    public function CheckVersionen($funktionen = null, $returnfirst = false)
    {
        ...
        if (strpos($result, 'ERROR') === false) {
                    ...
                    if (!$ioncube_loader_version) {
                        ...
                        if (strpos($versa[0], 'masterkey2') !== false) {
                            if ($phpversion && $phpversion[0] == '5' && $phpversion[2] < 6) {
                                ...
                                if (in_array($aktmasterkey, ['masterkey3',
                                    'masterkey4', ]) && ($phpversion[0] == '7' && $phpversion[2] == '0')) {
                                    $aktmasterkey = '';
                                }
            ...
            if (count($resulta) > 1) {
                ...
                foreach ($resulta as $resu) {
                    ...
                    if ($versa[0] === 'ent_masterkey4_20.3') {
                        $isVersion203Exists = true;
                    }
                    $i++;
                }
                $ret .= '</select>';
                $ret .= '<input class="button2" type="button" value="Updaten" id="upgrade" onclick="upgrade()"  />';
                if ($isVersion211Exists && $isVersion203Exists) {
    ...
    public function CopyFile($files, $maxtime = 10)
    {
    ...
    public function CheckVersandZahlungsweise($datei)
    {
    ...
    public function removeEmptyFile($file)
    {
    ...
    public function removeEmptyFolder($folder)
    {
    ...
    public function DownloadFile($files, $maxtime = 15, $echo = false)
    {
        $startzeit = microtime(true);
        $parameter['version'] = @$this->conf['version'];
        $parameter['versionname'] = @$this->conf['versionname'];
        ...
        if (count($rows) > 0) {
            foreach ($rows as $value) {
                ...
                if (is_file($file_lokal)) {
                    ...
                    } elseif ($eigenguser && $eigengroup) {
                        if ($fileowner != $eigenguser) {
                            if ($filegroup != $eigengroup) {
                                if (!$a) {
                                    return ['error' => 'ERROR Fehlende Schreibrechte in ' . $file_lokal];
                                }
                            } else {
                                if (!$g) {
                                    return ['error' => 'ERROR Fehlende Schreibrechte in ' . $file_lokal];
                    ...
                    if (is_file(dirname(__DIR__) . '/download/' . $file)) {
                        if (md5_file(dirname(__DIR__) . '/download/' . $file) != $md5sum) {
                            $res['download'][$downloadind] = ['typ' => $typ, 'file' => $file, 'md5sum' => $md5sum,
                                'size' => $size, ];
                            $downloadexists[$typ][$file] = $downloadind;
                            $downloadind++;
                        } else {
                            $res['copy'][$copyind] = ['typ' => $typ, 'file' => $file, 'md5sum' => $md5sum,
                                'size' => $size, ];
                        ...
                        if (md5_file(dirname(__DIR__) . '/download/' . $file) != $md5sum) {
                            ...
                            if (touch(dirname(__DIR__) . '/download/chkrights')) {
                                ...
                                if ($fileowner != $eigenguser) {
                                    if ($filegroup != $eigengroup) {
                                        if (!$a) {
                                            return ['error' => 'ERROR Fehlende Schreibrechte im Downloadordner'];
                                        }
                                    } else {
                                        if (!$g) {
                                            return ['error' => 'ERROR Fehlende Schreibrechte im Downloadordner'];
    ...
    public function CheckUpdateModules()
    {
        //$this->dir_rekursiv("./");
    ...
    public function CheckUpdateCustom()
    {
        $parameter['version'] = @$this->conf['version'];
    ...
    public function CheckDownloadedUpdate()
    {
    }
    ...
    public function Request($command, $parameter)
    {
        $erp = $this->erp;
    ...
    public function dir_rekursiv($verzeichnis)
    {
...
class UpdateWawiString
{
    ...
    public function BuildRegExp($array)
    {
        $regexp = '/^';
    ...
    public function FindPercentValues($pattern)
    {
        $hash = '';
        $collect = '';
    ...
    public function valid_utf8($string)
    {
...
class UpdatephpWFAPI
{
    ...
    public function ReBuildPageFrame()
    {
        $this->app->Tpl->ResetParser();
        $this->BuildPageFrame();
    }

    public function BuildPageFrame()
    {
    ...
    public function StartRequestedCommand()
    {
    ...
    public function EasyTableList($tablename, $cols, $parsetarget, $pkname, $delmsg, $delmsgcol)
    {
        // show list
    ...
    public function Message($msg, $parsetarget = 'MSGBOX')
    {
    ...
    public function EmailFromTemplate($template, $to, $values)
    {
...
class UpdateSecure
{
    ...
    public function GetGET($name, $rule = '', $maxlength = '', $sqlcheckoff = '')
    {
        return $this->Syntax(isset($this->GET[$name]) ? $this->GET[$name] : '', $rule, $maxlength, $sqlcheckoff);
    }

    public function GetPOST($name, $rule = '', $maxlength = '', $sqlcheckoff = '')
    {
    ...
    public function RuleCheck($value, $rule)
    {
        $v = $this->GetRegexp($rule);
        if (preg_match_all('/' . $v . '/i', $value, $teffer)) {
            return true;
        ...
        ');
            ...
            elseif ($hwtoken == 3) {
                ...
                WHERE username='" . $username . "' AND activ='1' LIMIT 1");
                        ...
                        } elseif ($check < $hwcounter && $check > 0) {
...
//          }
    ...
    public function mOTP($pin, $otp, $initsecret)
    {
...
class UpdateUser
{
    ...
    public function GetProjektleiter()
    {
    ...
    public function GetFirmaName()
    {
        if (isset($this->cache['firmaname'])) {
            return $this->cache['firmaname'];
        }
...
{
    public $NAME; //Name des Templates

    public $PATH; //PFAD des Templates

    public $parsed; //Zustand

    public $ORIGINAL; //Parse - Text Vorlage

    public $VARS; //assoziatives Array mit Variablennamen als Index

    public $Elements;

    public $vararraycreated;

    public function __construct($_path, $_file)
    {
    ...
    public function readFile()
    {
        ...
        if ($fp) {
            ...
            die($_path . $_file . ' not found');
        }
        $this->ORIGINAL = isset($contents) ? $contents : '';
        //$this->CreateVarArray();
    }

    public function CreateVarArray()
    {
        ...
        if (count($matches[0]) > 0) {
            ...
            for ($i = 0; $i < $cmatches; $i++) {
                ...
                $this->Elements[$i]['el'] = str_replace(']', '', $this->Elements[$i]['el']);
                if ($i > 0) {
                    $this->Elements[$i - 1]['nach'] = $this->Elements[$i]['before'];
                }
            }
    ...
    public function AddVar($_var, $_value)
    {
        $this->VARS[$_var] = $this->VARS[$_var] . $_value;
    }

    public function SetVar($_var, $_value)
    {
...
class UpdateTemplateParser
{
    ...
    public function GetVars($tplfile)
    {
        $fp = @fopen($tplfile, 'r');
        if ($fp) {
    ...
    public function ShowTemplates()
    {
        foreach ($this->TEMPLATELIST as $key => $value) {
            foreach ($value as $key1 => $text) {
                if (!is_array($text)) {
                    echo "${key} " . htmlspecialchars($text) . '<br>';
                }
                if (is_array($text)) {
    ...
    public function Set($_var, $_value, $variable = false)
    {
    ...
    public function Add($_var, $_value, $variable = false)
    {
    ...
    public function OutputAsString($_template)
    {
        return $this->app->erp->ClearDataBeforeOutput($this->Parse('', $_template, 1));
    }

    public function Parse($_var, $_template, $return = 0)
    {
    ...
    public function AddAndParse($_var, $_value, $_varparse, $_templateparse)
    {
        $this->Set($_var, $_value);
        $this->Parse($_varparse, $_templateparse);
    }

    public function ParseVarVars()
    {
        $pattern = '/((\[[A-Z0-9_]+\]))/';
    ...
    public function FinalParse($_template)
    {
        $this->ParseVarVars();
        if (isset($this->TEMPLATELIST[$_template]) && !($this->TEMPLATELIST[$_template]->vararraycreated)) {
            $this->TEMPLATELIST[$_template]->CreateVarArray();
...
class UpdateApplication
{
    ...
    public function ActionHandlerListen(&$app)
    {
        ...
        if ($action != '') {
            if (isset($this->ActionHandlerList[$action])) {
            ...
            if (isset($this->ActionHandlerDefault)) {
...
class UpdatePlayer
{
    ...
    public function Run($sessionObj)
    {
        ...
        $this->app->Tpl->Set(
            'LIZENZHINWEIS',
            ' <a href="https://xentral.com/lizenzhinweis" target="_blank">Lizenzhinweis</a>'
        );
        switch ($action) {
            case 'login':
                ...
                switch ($cmd) {
                    ...
                    case 'downloadfiles2':
                        ...
                        if ($version) {
                            ...
                            $files = false;
                            if ($version) {
                                $updateFileListResponse = $myUpd->CheckFiles(true);
                                $updateFileList = $updateFileListResponse['download'] ?? [];
                                $myUpd->DownloadFile($updateFileList);
                                $data2 = $myUpd->CheckFiles();
                                if (isset($data2['download'])) {
                                    $files = $data2['download'];
                                }
                            }
                            if ($version && $files) {
                    ...
                    case 'copyfiles2':
                        ...
                        if ($version) {
                            $updateFileListResponse = $myUpd->CheckFiles(true);
                            $updateFileList = $updateFileListResponse['copy'] ?? [];
                            $myUpd->CopyFile($updateFileList);
                            $data2 = $myUpd->CheckFiles();
                            if (isset($data2['download'])) {
                        ...
                        if ($version && $files) {
                            ...
                            if ($data['tocopy'] === 0 && function_exists('opcache_reset')) {
                                if (file_exists(dirname(__DIR__) . '/app/Core/Installer/Commands/ExecuteUpdateCommand.php')) {
                                    exec('cd ' . dirname(__DIR__) . ' && php refreshFileCache.php');
                                    $commandOutputs = [];
                                    $exitCode = 1337;
                                    exec('cd ' . dirname(__DIR__) . ' && php artisan xentral:update', $commandOutputs, $exitCode);
                                    echo json_encode([
                                        'completed' => true,
                                        'exitCode' => $exitCode,
                                        'commandOutput' => $commandOutputs,
                                    ]);
                                    exit;
                                }
                                echo json_encode($data);
                                exec('cd ' . dirname(__DIR__) . ' && php artisan optimize:clear');
                    ...
                    case 'upgradedb':
                        ...
                        foreach (['config.php', 'packages.php', 'routes-v7.php', 'services.php'] as $fileName) {
                            if (file_exists(LARAVEL_BOOSTRAP_CACHE_DIR . $fileName)) {
                                unlink(LARAVEL_BOOSTRAP_CACHE_DIR . $fileName);
                            }
                        }
                        ...
                        // Boot new application that the old application has access to all new registered functionalities
                        ...
                        if ($obj) {
                            ...
                            if (method_exists($obj, 'GetTMP')) {
                                ...
                                if (!empty($tmp)) {
                                    $oldTmp = dirname($tmp) . '/';
                                    foreach (['cache_services.php', 'cache_javascript.php',
                                        'cache_classmap.php', ] as $file) {
                                        // Aktuelle Cache-Dateien (MultiDB) löschen
                                        if (file_exists($tmp . $file)) {
                                            if (function_exists('opcache_invalidate')) {
                                                opcache_invalidate($tmp . $file, true);
                        ...
                        ob_start();
                        exec('cd ' . dirname(__DIR__) . ' && php refreshFileCache.php && php artisan optimize:clear');
                        $data['nr'] = $obj->UpgradeDatabase($nummer);
                        ob_end_clean();
