...
class OrderDeliveryAddressCast implements CastsAttributes
{
    /**
     * @param Model  $model
     * @param string $key
    ...
    public function get($model, string $key, $value, array $attributes): Address
    {
        ...
        if ($model->hasDifferentDeliveryAddress()) {
            return (new Address())
                ->setName($model->liefername)
                ->setCity($model->lieferort)
                ->setStreet($model->lieferstrasse)
                ->setZip($model->lieferplz)
                ->setStateCode($model->lieferbundesstaat)
                ->setAdditionalInformation($model->lieferaddresszusatz)
                ->setCountryCode($model->lieferland)
