...
final class EbayListingService
{
    ...
    public function reserveStockForListing(int $articleId, string $itemId, int $quantity, int $shopId): void
    {
        $sql = 'INSERT INTO `ebay_reserved_stock` 
            (`article_id`, `shopexport_id`, `external_order_id`, `quantity`, `order_date` ) 
            VALUES (:article_id, :shopexport_id,:external_order_id, :quantity, NOW())';
        $values = [
            'article_id'        => $articleId,
            'shopexport_id'     => $shopId,
            'external_order_id' => $itemId,
            'quantity'          => $quantity,
        ];
    ...
    public function deleteReservedStockForListing(string $itemId): void
    {
        $sql = 'DELETE FROM `ebay_reserved_stock`  WHERE `external_order_id` = :external_order_id';
        $values = [
            'external_order_id' => $itemId,
        ];
    ...
    public function recalculateReservedStockForArticleId(int $articleId, int $freeFieldNumber): void
    {
        $sql = "SELECT SUM(e.quantity) AS `quantity`
        ...
        $quantity = (int)$this->db->fetchValue($sql, $values);

        $freeField = 'freifeld'.$freeFieldNumber;
        $sql = "UPDATE `artikel` SET {$freeField} = :value WHERE `id` = :article_id";
        $values = [
            'value'      => $quantity,
            'article_id' => $articleId,
        ];
