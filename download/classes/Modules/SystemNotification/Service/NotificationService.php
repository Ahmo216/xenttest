...
final class NotificationService implements NotificationServiceInterface
{
    ...
    public function create(
        ...
        if (!in_array($type, self::$validMessageTypes, true)) {
            throw new InvalidArgumentException(sprintf(
                '"%s" is not a valid message type. Valid types are: %s',
                $type,
                implode(', ', self::$validMessageTypes)
            ));
        }
        ...
        // Sanitize buttons
        if (isset($options['buttons']) && is_array($options['buttons']) && count($options['buttons']) > 0) {
     ...
     */
    public function createFromData($userId, NotificationMessage $data)
    {
        return $this->create(
            $userId,
            $data->getType(),
