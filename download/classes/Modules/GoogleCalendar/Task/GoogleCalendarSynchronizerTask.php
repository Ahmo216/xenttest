...
use Xentral\Modules\SystemNotification\SystemNotificationInterface;
...
final class GoogleCalendarSynchronizerTask
{
    ...
    /** @var SystemNotificationInterface $notificationService */
     ...
     * @param SystemNotificationInterface $notificationService
     */
    public function __construct(
        ...
        SystemNotificationInterface $notificationService
    ) {
    ...
    public function execute(): void
    {
        ...
        try {
            ...
            foreach ($accounts as $account) {
                ...
                } catch (GoogleApiAccessException | AuthorizationExpiredException $e) {
                    $this->addLoginWarningNotification($account->getUserId());
                } catch (Exception $e) {
                    $this->logger->debug(
     ...
     * @return void
     */
    private function addLoginWarningNotification(int $userId): void
    {
        $message = new NotificationMessage(
            NotificationServiceInterface::TYPE_WARNING,
            'Google Verbindungsfehler',
            'Die Verbindung zu Google Kalender wurde getrennt, bitte verbinden Sie sich erneut.'
        );
        $message->addButton('zum Modul', 'index.php?module=welcome&action=settings');
        $this->notificationService->addNotification($userId, $message);
    }
