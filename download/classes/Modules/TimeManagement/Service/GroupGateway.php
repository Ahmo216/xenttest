...
final class GroupGateway
{
     ...
     * @return bool
     */
    public function isAnySupervisorInGroup(int $groupId): bool
    {
        $supervisorId = (int)$this->db->fetchValue("SELECT ar.adresse 
          FROM `adresse_rolle` AS `ar` 
          JOIN `user` AS `u` ON u.adresse = ar.adresse
          LEFT JOIN `userrights` AS `ur` ON u.id = ur.user
          WHERE ((ur.module = 'mitarbeiterzeiterfassung' AND ur.action = 'timemanagementhandle' AND ur.permission = 1) 
                  OR u.type = 'admin') 
            AND ar.subjekt = 'Mitglied' AND ar.objekt = 'Gruppe' AND (ar.bis = '0000-00-00' OR ar.bis >= CURDATE()) 
            AND ar.parameter = '$groupId' LIMIT 1");
