...
final class ImapMailClient implements MailClientInterface
{
     ...
     * @throws ClientConnectionException
     *
     * @return MailMessageOverview
     */
    public function fetchMessageOverview(int $msgNumber): MailMessageOverview
    {
        $this->ensureConnection();
        try {
            $rawHeader = $this->imap->getRawHeader($msgNumber);
        } catch (Exception $e) {
            throw new MessageNotFoundException(
                sprintf('Message number %s not found.', $msgNumber)
            );
        }
        $headers = Headers::fromString($rawHeader);

        return new MailMessageOverview(
            $msgNumber,
            null,
            new MailMessageHeaderCollection($this->parseHeaders($headers))
        );
    }
     ...
     * @return string[]
     */
    public function getFolderNames(string $rootFolder = null): array
    {
        $this->ensureConnection();
        try {
            $folder = $this->imap->getFolders($rootFolder);
        } catch (Exception $e) {
            throw new InvalidArgumentException("Folder '{$rootFolder}' not found.");
        }

        $returnValue = [];
        foreach ($folder as $subFolder) {
            $temp = $this->getFoldersRecursive($subFolder);
            $returnValue = array_merge($returnValue, $temp);
        }
     ...
     * @return array|string[]
     */
    private function getFoldersRecursive(Folder $rootFolder): array
    {
        $subFolderArray = [];
        foreach ($rootFolder as $child) {
            $temp = $this->getFoldersRecursive($child);
            $subFolderArray = array_merge($subFolderArray, $temp);
        }

        $subFolderArray = array_merge([$rootFolder->getGlobalName()], $subFolderArray);
