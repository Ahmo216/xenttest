...
final class MailMessageData implements MailMessageInterface, JsonSerializable
{
    ...
    public function getDate(): ?DateTimeInterface
    {
        ...
        try {
            return new DateTimeImmutable($date->getValue());
        } catch (Exception $e) {
        }
    ...
    /**
     * Searches the message part tree for attachments recursively.
     *
    ...
    private function findAttachmentParts(MailMessagePartInterface $part, array &$resultArray): void
    {
        ...
        $contentDisposition = $header !== null ? $header->getValue() : '';

        $headerParts = explode(';', $contentDisposition);
        ...
        if ($disposition === 'inline' && count($headerParts) > 1) {
            $resultArray[] = $part;
