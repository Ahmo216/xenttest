...
class Acl
{
    ...
    public function Check($usertype, $module, $action, $userid = '')
    {
        ...
        if ($module === 'welcome' && in_array($action, ['login', 'main', 'logout'])) {
            $ret = true;
        }
                ...
                if (
                    ...
                    switch ($module) {
                        ...
                        case 'dateien':
                            ...
                            if (array_search($dateiModul, ['auftrag', 'rechnung', 'lieferschein', 'bestellung',
                                    'angebot', 'verbindlichkiet', 'proformarechnung', 'anfrage', 'artikel', 'adresse',
                                    'produktion']) !== false) {
                    ...
                    $modact = ['artikel' => ['einkauf', 'dateien', 'eigenschaften', 'verkauf', 'statistik', 'etikett',
                        'offenebestellungen', 'offeneauftraege', 'zertifikate', 'fremdnummern']
                        , 'adresse' => ['rollen', 'ansprechpartner', 'lieferadresse', 'accounts', 'brief', 'belege',
                            'kundeartikel', 'abrechnungzeit', 'artikel', 'service', 'serienbrief']
  ...
  public function getEmailAddressFromUserAddress(int $userId, int $addressId): array
  {
    ...
    if($mailAddress !== ''
      && $mailAddress !== 'mail@ihr_mail_server.de'
      && $mailAddress !== erpAPI::Ioncube_Property('cloudemail')
    ) {
      $mailAddresses[] =  $mailAddress;
    }
   ...
   * @return string
   */
  public function getPseudonymizedEmail(string $mail): string
  {
    $mailParts = explode('@', $mail);
    $prefix = mb_strlen($mailParts[0]) < 3 ? $mailParts[0] : $mailParts[0][0] . '...' . substr($mailParts[0], -1);
    if(count($mailParts) === 1) {
      return $prefix;
    }
    $mailParts = explode('.', $mailParts[1]);
    $suffix = mb_strlen($mailParts[0]) < 3 ? $mailParts[0] : $mailParts[0][0] . '...' . substr($mailParts[0], -1);
    if(count($mailParts) > 1){
      $suffix .= '.' . $mailParts[count($mailParts) - 1];
    }
   ...
   * @return string
   */
  public function getEmailSuccessMessage(?string $mail): string
  {
    if($mail === null){
      return '';
    }

    return __('Es wurde eine Mail an :mail gesendet.', ['mail' => $this->getPseudonymizedEmail($mail)]) . ' ';
  }
  ...
  public function Passwortvergessen()
  {
    ...
    if ($code) {
        ...
        if ($aendern) {
          ...
          if ((string)$passwort !== '') {
            if ($passwort === $passwortwiederholen) {
              ...
              $errorMessage = __('Das Passwort muss mindestens 6 Zeichen besitzen.');
            } else {
              $errorMessage = __('Die Passwörter stimmen nicht überein.');
            }
          } else {
            $errorMessage = __('Bitte ein Passwort eingeben.');
          }
        }
        $userName = $this->app->DB->Select("SELECT `username` FROM `user` WHERE `id` = '$user' LIMIT 1");
      } else {
        $errorMessage = __('Dein Link ist nicht mehr gültig.');
      }
    } else {
      if ((string)$vergessenusername !== '') {
        ...
        if(!empty($emailAddresses)) {
          ...
          !$this->app->DB->Select(
            ...
            if ($pos) {
              ...
              $server = $server . '/index.php?module=welcome&action=passwortvergessen&code=' . $code;
            }
            ...
            foreach(['default', 'fallback'] as $sentSetting) {
              ...
              foreach ($emailAddresses as $email) {
                ...
                if($this->app->erp->MailSend(
                  ...
                  $successfullySentTo = $recipientMailAddress;
                  break 2;
                }
              }
            }
        ...
        if($successfullySentTo !== null || $userId <= 0) {
          $infoMessage = $this->getEmailSuccessMessage($successfullySentTo)
          . __('Bitte prüfen Sie Ihr E-Mail-Postfach. Falls keine E-Mail angekommen ist, wenden Sie sich bitte an den Administrator.');
        }
        elseif(empty($emailAddresses)) {
          $infoMessage = __('Es ist keine Email hinterlegt. Bitte wenden Sie sich an den Administrator.');
        }
        else{
          $infoMessage = __('Es ist ein Fehler beim Senden der Email aufgetreten. Bitte wenden Sie sich an den Administrator.');
        }
      }
    }

    return view('modules.welcome.passwort-vergessen', compact('userName', 'infoMessage', 'errorMessage'));
  }
    ...
    {
        $loginLocked = $this->IsInLoginLockMode();
        $loginErrorMessage = false;
        $multiDatabases = $this->app->getDbs();
        $resetStorage = false;
        ...
        if ($username == '' && ($password == '' || $token == '') && $stechuhrdevice == '' && $rfid == '') {
            ...
            return view('modules.welcome.login', compact('loginLocked', 'loginErrorMessage', 'multiDatabases', 'resetStorage'));
        }
        ...
        if ($externlogin == 0 && $localconnection == 0) {
            $loginErrorMessage = __('Es ist kein externer Login mit diesem Account erlaubt.');
            return view('modules.welcome.login', compact('loginLocked', 'loginErrorMessage', 'multiDatabases', 'resetStorage'));
        ...
        if ($fehllogins >= 8) {
            $loginErrorMessage = __('Max. Anzahl an Fehllogins erreicht. Bitte wenden Sie sich an Ihren Administrator.');
            return view('modules.welcome.login', compact('loginLocked', 'loginErrorMessage', 'multiDatabases', 'resetStorage'));
        }
        ...
        $loginErrorMessage = __('Benutzername oder Passwort falsch.');
        return view('modules.welcome.login', compact('loginLocked', 'loginErrorMessage', 'multiDatabases', 'resetStorage'));
    }
